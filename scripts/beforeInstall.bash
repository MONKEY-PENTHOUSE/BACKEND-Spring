...
docker stop monkeypenthouse_backend
docker rm monkeypenthouse_backend
...
if [[ "$(docker images -q gmlwls3520/BACKEND-Spring:v1 2> /dev/null)" != "" ]]; then
docker rmi -f $(docker images --format '{{.Repository}}:{{.Tag}}' --filter=reference='gmlwls3520/BACKEND-Spring:v1')
fi
